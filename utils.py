...
def getFaceAngle(shapes2D):
    ...
    leftCheekLen=math.sqrt(math.pow(shapes2D[0][0][31]-shapes2D[0][0][2],2)+math.pow(shapes2D[0][1][31]-shapes2D[0][1][2],2))
    #오른쪽 볼 길이 (15번 포인트 - 36번 포인트)
    rightCheekLen=math.sqrt(math.pow(shapes2D[0][0][14]-shapes2D[0][0][35],2)+math.pow(shapes2D[0][1][14]-shapes2D[0][1][35],2))
    
    #print("leftCheekLen: {}  rightCheekLen: {}".format(leftCheekLen,rightCheekLen))
   
    #얼굴 수평 길이 (15번 포인트 - 3번 포인트)
    faceParallelLen=math.sqrt(math.pow(shapes2D[0][0][14]-shapes2D[0][0][2],2)+math.pow(shapes2D[0][1][14]-shapes2D[0][1][2],2))
    # (비율) 왼쪽 볼 길이 / 얼굴 수평 길이
